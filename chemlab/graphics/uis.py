...
def setup_textures():
    ...
    for i in range(nglyphs):
        ...
        actual_coords.append((float(glyph_width) / width, float(glyph_height) / height))

    return dict(
        textures=textures, sizes=sizes, coords=actual_coords, geometrics=geometrics
    )
...
class TextUI(object):
    ...
    **Parameters**
    ...
    def __init__(self, widget, x, y, text):
        ...
        vert = open(
            os.path.join(curdir, "postprocessing", "shaders", "text.vert")
        ).read()
        frag = open(
            os.path.join(curdir, "postprocessing", "shaders", "text.frag")
        ).read()
    ...
    def draw(self):
        ...
        self.xy_ndc = np.array(
            [
                (float(self.x) / self.res_x) * 2 - 1,
                (float(self.y) / self.res_y) * 2 - 1,
            ],
            "float32",
        )
        ...
        for c in self.text:
            ...
            wh_ndc = np.array(
                [(float(w) / self.res_x) * 2, (float(h) / self.res_y) * 2], "float32"
            )
            ...
            tex_data = np.array(
                [0, 0, 1.0, 0, 0, 1.0, 0, 1.0, 1.0, 0, 1.0, 1.0], dtype="float32"
            )
