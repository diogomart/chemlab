...
void main() {
  vec2 invTexSize = texcoordOffset;
...
for(int i = -uSample; i < uSample; i++){
    for(int j = -uSample; j < uSample; j++){
      vec2 off = vec2(float(i)*invTexSize.x, float(j)*invTexSize.y) * uScale;
      depth = texture2D(s_depth, pos + off);
      sample_norm = texture2D(s_norm, pos + off).xyz * 2.0 - 1.0;
